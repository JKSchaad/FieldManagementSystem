    ...
    initialize: function() {
        ...
        i18next
            ...
            .init({
                ...
                backend: {
                    loadPath: 'lib/custom/i18n/{{lng}}/{{ns}}.json'
                }
            })
            .then(function() {
            ...
            i18next.changeLanguage(newLanguage.code)
                .then(function() {
                    ...
                    if (newLanguage.dir == 'rtl') {
                        app.$('.f7[rel=stylesheet]').attr('href', 'lib/vendor/framework7/framework7-bundle-rtl.min.css');
                        app.rtl = true;
                        app.params.rtl = true;
                    }
                    else {
                        app.$('.f7[rel=stylesheet]').attr('href', 'lib/vendor/framework7/framework7-bundle.min.css');
                        app.rtl = false;
...
app.on('languageChanged', function(language) {
    ...
    if ('moment' in window) {
        if (moment.locales().includes(language.code)) {
            ...
            LazyLoad.js([`lib/vendor/moment/locale/${language.code}.js`], function() {
                moment.locale(language.code);
            });
        }
    }
